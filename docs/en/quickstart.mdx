---
title: "Quick Start"
description: "Get claude-meta-settings running in minutes and configure your first MCP servers"
---

Get claude-meta-settings up and running in just a few minutes and configure your first MCP server aggregation.

<Note>
This quick start uses Docker. To run claude-meta-settings directly on a host without Docker
(Node + PostgreSQL), see [Running without Docker](/en/deployment/native-deployment).
</Note>
...
<AccordionGroup>
  <Accordion icon="github" title="Clone the Repository">
    Clone claude-meta-settings from GitHub and navigate to the project directory:
  ...
  <Accordion icon="settings" title="Configure Environment">
    ...
    <Note>
      If you modify `APP_URL` in the `.env` file, make sure you only access claude-meta-settings from that URL due to CORS policy enforcement.
    </Note>
  ...
  <Accordion icon="docker" title="Start with Docker Compose">
    Launch claude-meta-settings using Docker Compose:
    ...
    This will start:
    - claude-meta-settings fullstack docker image
    - PostgreSQL database
...
## Step 2: Access claude-meta-settings
...
4. **(Optional) Disable signup** You may only allow you and your team access claude-meta-settings, so once you setup the accounts, you can disable new user registration in Settings page.
...
<AccordionGroup>
  <Accordion icon="key" title="Create API Key">
    ...
    **Key Information:**
    - **Description**: `My first claude-meta-settings API key for news endpoint`
    - **Scope**: 
...
<CardGroup cols={2}>
  ...
  <Card title="Connect to Cursor" icon="mouse-pointer-2" href="/en/integrations/cursor">
    Configure Cursor IDE to use your claude-meta-settings endpoints
  </Card>
  
  <Card title="Connect to Claude Desktop" icon="monitor-dot" href="/en/integrations/claude-desktop">
    Set up Claude Desktop with claude-meta-settings using mcp-proxy
  </Card>
</CardGroup>
...
<AccordionGroup>
  <Accordion icon="warning" title="Common Issues">
    **CORS errors**: Ensure you're accessing claude-meta-settings from the URL specified in `APP_URL`.
    
    **Database connection**: Check that PostgreSQL container is running with `docker ps`.
    
    **Memory issues**: claude-meta-settings requires at least 2GB RAM for optimal performance.
  ...
  <Accordion icon="question" title="Need Help?">
    - Browse [GitHub Issues](https://github.com/Kirchlive/claude-meta-settings/issues)
  </Accordion>
</AccordionGroup>
