This guide walks you through deploying claude-meta-settings on a DigitalOcean VPS running Ubuntu from scratch, as an example.

<Note>
This guide uses Docker. To run claude-meta-settings directly on a host without Docker, see
[Running without Docker](/en/deployment/native-deployment).
</Note>
...
claude-meta-settings requires at least **2GB-4GB of memory** for optimal performance. The larger the instance, the better the performance due to MCP server pre-allocation and Docker operations.
...
## Step 3: Deploy claude-meta-settings

### 3.1 Clone the Repository

```bash
cd /opt
sudo git clone https://github.com/Kirchlive/claude-meta-settings.git
...
### 3.4 Start claude-meta-settings
