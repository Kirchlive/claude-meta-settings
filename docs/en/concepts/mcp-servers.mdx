---
title: "MCP Servers"
description: "Learn how to configure and manage MCP server instances in claude-meta-settings"
---

A **MCP Server** is a configuration that tells claude-meta-settings how to start and manage a Model Context Protocol server. These servers provide tools, resources, and prompts that can be aggregated and exposed through claude-meta-settings endpoints.

## What is a MCP Server?

MCP Servers are the building blocks of claude-meta-settings. Each server configuration defines:
...
claude-meta-settings supports three types of MCP servers:
...
1. **Navigate** to MCP Servers in the claude-meta-settings dashboard
2. **Click** "Add Server" 
...
claude-meta-settings supports bulk import and export of MCP server configurations for easy migration and backup.
...
claude-meta-settings pre-allocates idle sessions for better performance:
...
If your MCP servers require additional dependencies beyond `uvx` or `npx`, you can customize the claude-meta-settings Dockerfile:
...
<AccordionGroup>
  <Accordion icon="triangle-alert" title="Server Won't Start">
    ...
    **Debug steps:**
    1. Check server logs in claude-meta-settings dashboard
