...
**Middleware** in claude-meta-settings allows you to intercept and transform MCP requests and responses at the namespace level. This powerful feature enables you to add functionality like filtering, logging, validation, and security without modifying individual MCP servers.
...
claude-meta-settings provides several built-in middleware options:
...
<CardGroup cols={2}>
  <Card title="Inspector" icon="magnifying-glass" href="/en/concepts/inspector">
    Learn about the claude-meta-settings inspector for debugging middleware
  </Card>
