...
description: "Configure Claude Desktop to connect to claude-meta-settings endpoints using mcp-proxy"
---

**Claude Desktop** integration allows you to access claude-meta-settings tools directly through Claude's interface. Since Claude Desktop only supports stdio servers, you'll need a local proxy to connect to claude-meta-settings's remote endpoints.
...
Since claude-meta-settings endpoints are remote only (SSE, Streamable HTTP), Claude Desktop needs a local proxy to connect. Based on testing, `mcp-proxy` is the recommended solution for API key authentication.
...
For remote claude-meta-settings instances, simply replace the localhost URL:

```json
{
  "mcpServers": {
    "claude-meta-settings-Production": {
      "command": "uvx",
      "args": [
        "mcp-proxy",
        "--transport",
...
- For detailed troubleshooting, see [issue #76](https://github.com/Kirchlive/claude-meta-settings/issues/76) 