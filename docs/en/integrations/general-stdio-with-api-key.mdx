...
description: "Configure any stdio-based MCP client to connect to claude-meta-settings endpoints using mcp-proxy"
---

**General Stdio Integration** allows any MCP client that supports stdio servers to connect to claude-meta-settings's remote endpoints. Since claude-meta-settings endpoints are remote-only (SSE, Streamable HTTP), you'll need a local proxy to bridge the connection.
...
Since claude-meta-settings endpoints are remote only, you need `mcp-proxy` to convert between stdio and HTTP/SSE protocols.
...
1. **Connection refused**: Ensure claude-meta-settings is running and accessible
...
- **Replace** `sk_mt_your_api_key_here` with your claude-meta-settings API key
...
<CardGroup cols={2}>
  <Card title="General Stdio Integration using `mcp-remote` with OAuth` with API Key" icon="globe" href="/en/integrations/general-stdio-with-oauth">
    Configure any stdio-based MCP client to connect to claude-meta-settings endpoints using mcp-remote with OAuth
  </Card>
