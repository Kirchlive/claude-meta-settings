...
description: "Configure any stdio-based MCP client to connect to claude-meta-settings endpoints using mcp-remote with OAuth authentication"
---

**General Stdio Integration with OAuth** allows any MCP client that supports stdio servers to connect to claude-meta-settings's remote endpoints using OAuth authentication. Since claude-meta-settings endpoints are remote-only (SSE, Streamable HTTP), you'll need a local proxy to bridge the connection.
...
- **claude-meta-settings** running locally or deployed with OAuth enabled
- **Active endpoint** configured in claude-meta-settings
- **OAuth application** registered in claude-meta-settings
...
Since claude-meta-settings endpoints are remote only, you need `mcp-remote` to convert between stdio and HTTP/SSE protocols with OAuth authentication.
...
- **No manual OAuth setup** required - claude-meta-settings handles it automatically
...
<CardGroup cols={2}>
  ...
  <Card title="General Stdio Integration using `mcp-proxy` with API Key" icon="globe" href="/en/integrations/general-stdio-with-api-key">
    Configure any stdio-based MCP client to connect to claude-meta-settings endpoints using mcp-proxy
  </Card>
